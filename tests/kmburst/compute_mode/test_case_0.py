...
def test_case_0():
    ...
    burst,block_gt = cmode_setup(k,t,h,w,c,ps,std,device)
    ...
    # -- execute test --
    exec_test(CMODE_TYPE,0,k,t,h,w,c,ps,nblocks,nbsearch,nfsearch,kmeansK,
    ...
    modes_gt = torch.ones_like(zinits.modes)
    
    # -- compare results --
    delta = torch.sum(torch.abs(modes - modes_gt)).item()
