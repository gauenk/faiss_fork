...
// same as "chooseTileSize" but for image patches like in ImagePatchDistances
// If the inner size (dim) of the vectors is small, we want a larger query tile
// size, like 1024
inline void chooseImageTileSize(
        int height,
        int width,
        int nftrs,
        int patchsize,
    ...
    // If the k size (vec dim) of the matrix multiplication is small (<= 32),
    // increase to 1024.
    // int preferredTileHeight = 512;
    // if (nftrs <= 32) {
    //     preferredTileHeight = 1024*1024;
    // }
    // int preferredTilePix = std::min(targetUsage / preferredTileRows, width);
