  ...
  namespace gpu {
	...
	for (int comp = 0; comp < numOfComps; ++comp){
	  ...
	  if (inVal < outVal_max){
	    ...
	    for (int fidx = 0; fidx < nframes; ++fidx){
	      outKeys[fidx][row][col][kidx][0] = (int)inKeys[comp][row][col][fidx][0];
	      outKeys[fidx][row][col][kidx][1] = (int)inKeys[comp][row][col][fidx][1];
	    }
	...
	cudaStream_t stream){
      ...
      FAISS_ASSERT(outKeys.getSize(1) == inKeys.getSize(1)); // height
      FAISS_ASSERT(outKeys.getSize(2) == inKeys.getSize(2)); // width
